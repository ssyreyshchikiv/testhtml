$(document).ready(function(){
	...
	$(".progressbar li").on("click", function(){
		...
		$(".progressbar li").each(function(){
			// Если найли, то ставим триггер в положение фолс
			if($(this).hasClass("fail")){
				$trigger = true;
			}
		});
		// Проверяем завершенность предыдущего этапа
		if($(this).prev().hasClass("progresel") && 
			!$(this).prev().hasClass("complete") ||
			$(this).prev().hasClass("preparing")){
			return false;
		}
		if(!$(this).next().hasClass("progresel")){
			return false;	
		}
...
var eventMy = function(step, action){
	...
	switch(action){
		...
		case "2":
			$classSt  = "preparing";
			$classCat = "circle-color-orange";
			break;
		case "3":
			$classSt  = "fail";
			$classCat = "circle-color-red";
			break;
		default:
